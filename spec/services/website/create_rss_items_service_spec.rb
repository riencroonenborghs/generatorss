...
RSpec.describe Website::CreateRssItemsService, type: :service do
  let(:website) { create :website, last_loaded: nil }
  subject { described_class.call(website: website) }
  ...
  describe "#call" do
    ...
    context "when things go good" do
      let(:data) { load_data("websites/rss_entries.xml") }
      ...
      context "when some RSS entries exist" do
        it "creates only the new RSS entries" do
          ...
          expect { subject }.to change(website.rss_items, :count).by(10)
        end
      end

      context "when no RSS entries exist" do
        it "creates all the RSS entries" do
          expect { subject }.to change(website.rss_items, :count).by(15)
        end
      end
    end
  end
